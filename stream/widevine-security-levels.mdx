...
Widevine DRM enforces different content-quality tiers based on the security capabilities of the playback device. Understanding the L1, L2, and L3 levels — and their reliance on a Trusted Execution Environment — helps you set realistic expectations for which devices can stream HD or higher content.
...
<Warning>
   **DRM protection is only guaranteed with the default Bunny Player.** MediaCage DRM and Enterprise DRM are validated to work with the standard, unmodified Bunny Player. If you customize or modify the player including altering its source, replacing playback components, injecting custom scripts, or integrating a third-party player we cannot guarantee that DRM protection will function correctly or remain effective. Any resulting loss of content protection is outside Bunny's responsibility. 
</Warning>