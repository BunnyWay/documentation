...
<Update label="2026-07-16" tags={["Stream","(Improvement)"]}>
  ...
  We've just updated the Bunny player. Chromecast now supports:

  eDRM videos— Widevine-protected content plays on the TV, closing a long-standing gap vs the legacy player

  A native-feeling TV experience — video title and thumbnail on screen, player colors and captions appearance (color, font, size, language) synced from the embed

  Full playback control — audio track selection works before and during casting, and playback rate changes apply on the TV

  Seamless session handling — playback continues from the current position when casting starts and transfers back on stop; page reloads, replays after video end, and frozen-session edge cases are all handled [Learn more](/stream/)
</Update>
