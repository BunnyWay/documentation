...
<Update label="2026-06-23" tags={["Stream","(Improvement)"]}>
  ## Updated MediaCage Enterprise DRM multi-key licensing and billing documentation

  MediaCage DRM documentation now accurately reflects multi-key licensing behavior: a single video playback can result in multiple DRM licenses (e.g., separate licenses for video and translated audio tracks, with renewals based on video duration). DRM license billing examples and the MediaCage Basic DRM guide have also been updated with clearer Enterprise DRM recommendations for premium and pay-per-view use cases. [Learn more](/stream/drm)
</Update>
