...
Indicates how engaging a specific video is on a 0–100 scale based on viewing duration and viewing patterns. Higher values reflect stronger viewer retention. It is reported at the video level only and may be unavailable when there is not enough viewing data. This feature is only available over API and not available within the bunny dashboard.

<Note>
  [All stream statistics are available over the Stream API](https://docs.bunny.net/api-reference/stream/manage-videos/get-video-statistics)
</Note>