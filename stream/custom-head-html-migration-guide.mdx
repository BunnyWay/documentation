...
The new Bunny Stream Player uses a modern Web Components architecture (`media-chrome` + `bunny-*` components) instead of legacy Plyr internals.
...
| `.plyr__captions`            | `bunny-captions-display` and cue parts (`[part='cue-display']`, `[part='cue']`) |
