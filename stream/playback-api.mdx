...
The player can change playback speed, but the hosted player.js library only builds methods for the standard player.js names, so `player.setPlaybackRate()` is `undefined`. Send the command instead:

```js
player.send({ method: "setPlaybackRate", value: 1.5 });
```

Listen for `playbackratechange` to read the rate back. The callback receives the new rate:
...
`playbackratechange` - Fires when the playback rate changes. The callback receives the new rate. See [Playback rate](#playback-rate) for how to set it.

`error` - Fires when a command fails. Example:
...
| Code | Meaning |
| --- | --- |
| `2` | The method name is not valid. |
| `3` | The method is not supported by this player. |
| `5` | The underlying call threw. Most often a `play()` the browser refused because autoplay was blocked. |

## Requirements

The control API only works when the player runs inside an iframe. Nothing is sent on a direct playback URL.

The player accepts messages only from the parent origin it sees in `document.referrer`. A page that suppresses the referrer, such as one using `referrerpolicy="no-referrer"`, cannot control the player. This is the same policy that governs the library domain allow-list, so set it on the iframe as described in [Embedding restrictions](/stream/embedding#embedding-restrictions):
