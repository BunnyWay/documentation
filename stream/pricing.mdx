...
Transcribing is charged per language x per minute of video at the point the transcription service runs, this means total cost scales with both the total length of the video and the number of languages selected meaning: five languages selected = five times total cost based on video length. 

When you enable transcribing on a video library, it applies to all videos uploaded after that point. It does not apply to videos uploaded earlier.

You can also request transcription for a single video using [Transcribe video](/api-reference/stream/manage-videos/transcribe-video). This request overrides the library's transcription settings. Because of this, a transcribing charge can appear even when the library's dashboard setting is currently off. This can happen for two reasons:

1) The video was processed before the transcribing setting was turned off.
2) Transcription was requested directly through the API or the dashboard.

To view the total transcribed minutes for a video library check your video library's transcribing settings statistics graph within the dashboard or try the [Get transcribing statistics](/api-reference/core/stream-video-library/get-video-library-transcribing-statistics) API endpoint.
