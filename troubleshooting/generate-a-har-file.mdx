...
sidebarTitle: HAR File
description: Capture a HAR file from your browser so bunny.net support can see exactly what your browser sent and received.
---

A HAR (HTTP Archive) file is a JSON recording of every request your browser made during a session, including URLs, headers, timings, and response data. When support asks for one, it's the fastest way for us to see what actually happened instead of guessing from a description.
...
If the problem only happens on a specific hostname, Pull Zone, or region, reproduce it exactly that way. A capture of a working request tells us very little.
  ...
  <Tab title="Chrome, Edge & Brave">
    <Steps>
      <Step title="Open the affected page">
        Navigate to the page or application where you're seeing the issue.
      </Step>
      <Step title="Open developer tools">
        Press `F12`, or `Ctrl+Shift+I` on Windows and Linux, or
        `Cmd+Option+I` on macOS. You can also right-click the page and choose
        **Inspect**.
      </Step>
...
- The exact URL that failed, and the hostname or Pull Zone it belongs to
- The time the capture was taken, including your time zone
- The `CDN-RequestId` response header value, if you have it. It lets us find the request in our logs.
