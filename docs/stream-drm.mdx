...
\**Screen Grab Protection is not available on Widevine clients with L3 level of security. For a deeper understanding, refer to [Widevine DRM Security Levels](/docs/widevine-drm-security-levels).*
...
* **Session-based content key protection**:  The content is encrypted with an unique key for each play session.

**MediaCage Basic DRM** is suitable for scenarios where basic content protection is required. It is a innovative solution for users seeking entry-level security measures for their digital content.
...
In comparison to our MediaCage Basic DRM, the MediaCage Enterprise DRM protects the content also on client device using hardware vendor support for protecting content key during the playback.
...
* **Premium content licensing**: Ensures that a valid play license is issued only to authenticated clients that meet protection level requiremets.
...
<Info>
  To learn more about FairPlay streaming, see official [FPS website](https://developer.apple.com/streaming/fps/).
</Info>
...
Widevine provides digital media solutions for the delivery of digital entertainment to wide range of device platforms. Widevine is integrated into our MediaCage Enterprise DRM solution. The technology is built-in a number of device platforms such as Windows, Android, ChromeOS and browsers such as Chrome, Firefox, Edge...
...
* We count one DRM license for each unique VoD (Video on Demand) content, per device.
