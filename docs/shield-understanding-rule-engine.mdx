...
- **`GEO`** (_Optional_ **Variable Value** - **_COUNTRY_CODE, LATITUDE, LONGITUDE, ASN, CITY, CONTINENT_**) (**Advanced**) The geo location information of the client making the request.
...
- **`CONTAINSWORD`** Check if a string contains a specific word.
- **`WITHIN`** Check if a string is within a specified substring.
- **`STRMATCH`** Check if a string has exact match.
...
[Creating a custom WAF rule](/docs/shield-creating-a-custom-waf-rule)
