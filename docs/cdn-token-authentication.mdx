...
<CodeGroup>```bash bash &expires=1598024587 ```</CodeGroup>
...
<CodeGroup>
  ```bash bash https://test.b-cdn.net/my-partial/url/playlist.m3u8 ```
</CodeGroup>
...
<CodeGroup>```bash bash /my-partial/url/playlist.m3u8 ```</CodeGroup>

However, by passing the token_path for the following parameter:

<CodeGroup>```bash bash &token_path=%2Fmy-partial%2Furl%2F ```</CodeGroup>
...
<CodeGroup>```bash bash &token_countries=SI,GB ```</CodeGroup>
...
<CodeGroup>```bash bash &token_countries_blocked=SI,GB ```</CodeGroup>
...
<CodeGroup>```bash bash &limit=1024 ```</CodeGroup>
...
<CodeGroup>
  ```bash bash Base64Encode( SHA256_RAW(token_security_key + signed_url +
...
<CodeGroup>
  ```bash bash "param1=something&param2=something&param3=something" ```
</CodeGroup>
...
[Token authentication (basic)](/docs/cdn-token-authentication-basic)
