...
<Update label="2026-07-30" tags={["Magic Containers","(Improvement)"]}>
  ## Increased application limit for Standard accounts

  Standard (production) Magic Containers accounts can now run up to 20 applications per account, doubled from the previous limit of 10. Trial account limits and per-region pod limits remain unchanged. [Learn more](/magic-containers/limits)
</Update>
