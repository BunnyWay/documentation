...
<Update label="2026-08-14" tags={["Storage","(Improvement)"]}>
  ...
  The S3-compatible API now supports virtual hosted-style URLs (`https://bucket-name.[region]-s3.storage.bunnycdn.com/key`) alongside path-style URLs. HeadObject and GetObject return `ETag` and support conditional operations (`If-Match`, `If-None-Match`, `If-Modified-Since`, `If-Unmodified-Since`). CopyObject preserves Content-Type, returns `ETag`, supports `x-amz-metadata-directive`, and accepts conditional copy headers. Rate-limited requests return a `SlowDown` (503) response with a `Retry-After` header. [Learn more](/storage/s3)
</Update>
