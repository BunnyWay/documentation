...
<Update label="2026-06-25" tags={["CDN", "(Improvement)"]}>
  ## Wildcard certificates, ACME passthrough, and seamless migration for SSL

  The CDN SSL setup guide now covers automatic wildcard certificate issuance via Bunny DNS, ACME challenge passthrough to origin servers, and using Seamless Domain Migration for zero-downtime certificate provisioning before repointing your domain. [Learn more](/cdn/ssl-setup)

</Update>

<Update label="2026-06-25" tags={["Integrations", "(New)"]}>
  ## Bunny Shield, Offloader, and DNS guides for the WordPress plugin

  New WordPress plugin documentation for Bunny Shield (WAF and DDoS protection), a dedicated Bunny DNS and CDN Acceleration prerequisite guide, and a streamlined Offloader setup page. The WordPress integration overview now lists Shield alongside existing plugin features. [Learn more](/integrations/wordpress/shield)
