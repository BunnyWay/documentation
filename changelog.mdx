...
<Update label="2026-06-08" tags={["CLI", "(New)"]}>
  ## CLI: Edge Scripts, DNS, and shell completion commands

  The Bunny CLI now includes full Edge Scripts management (`bunny scripts`): create, deploy, delete scripts, manage deployments and rollbacks, environment variables and secrets, and custom domains with SSL. A new experimental `bunny dns` command lets you manage DNS zones and records, including DNSSEC, query logging, and BIND zone file import/export. Shell tab completion is also now available via `bunny completion`. [Learn more](/cli)
</Update>
