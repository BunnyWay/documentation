...
<Update label="2026-05-20" tags={["Stream","(Improvement)"]}>
  ## Stream Video storage size info param & Dashboard update

  We've created and extended the 'Get Video Storage Size Info' endpoint, giving customers a granular, per-video breakdown of your encoded videos storage consumption over API.

  This feature has been added to the Stream dashboard for granular video storage usage breakdown via the (i) information icon - selectable within your video library or the video page. [Dashboard Learn more](/stream/encoding) [API Learn more](/api-reference/stream/manage-videos/get-video-storage-size-info)
</Update>
