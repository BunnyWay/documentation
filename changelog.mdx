...
<Update label="2026-08-05" tags={["Magic Containers","(New)"]}>
  ## Bunny Container Registry (Preview)

  Push container images to a private, OCI-compliant registry at `registry.bunny.net` and deploy them with Magic Containers. Images can be pushed with Docker or the [bunny CLI](/cli/commands/registry), and automatically appear in the Magic Containers image list. No external registry or credential setup required. [Learn more](/magic-containers/bunny-container-registry)
</Update>
