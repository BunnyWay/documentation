...
<Info>
  S3 compatibility is currently in public preview. Currently, S3 compatibility can only be enabled when creating a new storage zone and cannot be enabled on an existing storage zone. Support for enabling S3 compatibility on existing storage zones is currently in development.
</Info>
...
- **Region**: `de` (Frankfurt), `ny` (New York), `sg` (Singapore), `uk` (London), <br />`se` (Stockholm), `la` (Los angeles),`jh` (Johannesburg) & `syd` (Sydney)

<Note>
  Bunny Storage supports both path-style URLs (e.g., `https://[region]-s3.storage.bunnycdn.com/bucket-name/key`) and virtual hosted-style URLs (e.g., `https://bucket-name.[region]-s3.storage.bunnycdn.com/key`).
</Note>
...
**Presigned URL example (1hour):**<br />https://\[region\]-s3.storage.bunnycdn.com/bucket-name/path/to/object/?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=\[AccessKeyID\+Date\+region\+servicerequest\]&X-Amz-Date=20260320T160912Z&X-Amz-Expires=3600&X-Amz-SignedHeaders=host&X-Amz-Signature=\[HMAC-SHA256Signature\]/
...
- **DELETE Object**: Delete objects (single object only — batch `DeleteObjects` is not supported)
...
| ✅ [`UploadPartCopy`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_UploadPartCopy.html) | ✅ Copy within same storage zone<br />✅ Part size up to 5 GB<br />✅ Range copy (source object must be greater than 5 MB)<br />✅ Supported headers:<br />    ✅ x-amz-copy-source<br />    ✅ x-amz-copy-source-range (optional, format: bytes=first-last)<br />✅ Conditional source headers:<br />    ✅ x-amz-copy-source-if-match<br />    ✅ x-amz-copy-source-if-modified-since<br />    ✅ x-amz-copy-source-if-none-match<br />    ✅ x-amz-copy-source-if-unmodified-since<br />❌ Cross-zone copying<br />❌ Server-side encryption<br />❌ Bucket-owner verification<br />❌ Requester payer<br />❌ ACL grants |
...
| ✅ [`GetObject`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_GetObject.html) | ✅ Full object download<br />✅ Response headers:<br />    ✅ Content-Type<br />    ✅ Content-Length<br />    ✅ Last-Modified<br />    ✅ ETag<br />✅ Range requests (byte ranges)<br />✅ Multipart object download<br />✅ Conditional Operations:<br />    ✅ If-Match<br />    ✅ If-Modified-Since<br />    ✅ If-None-Match<br />    ✅ If-Unmodified-Since<br />❌ SSE-C encryption<br />❌ Versioning support |
...
| ✅ [`CopyObject`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CopyObject.html) | ✅ Object copy within same storage zone<br />✅ Object size up to 5 GB<br />✅ Response fields:<br />    ✅ LastModified<br />    ✅ ChecksumSHA256 (Base64 encoded)<br />✅ Source object checksum validation<br />✅ Content-Type preservation<br />✅ ETag in response<br />✅ Metadata directive:<br />    ✅ x-amz-metadata-directive (custom user metadata is not copied, as custom metadata is not yet supported)<br />✅ Conditional copy operations:<br />    ✅ x-amz-copy-source-if-match<br />    ✅ x-amz-copy-source-if-modified-since<br />    ✅ x-amz-copy-source-if-none-match<br />    ✅ x-amz-copy-source-if-unmodified-since<br />❌ Cross-zone copying<br />❌ Storage class selection<br />❌ SSE/SSE-C encryption<br />❌ Tagging |
...
Additional regions will be added in the future. Regions that do not yet support S3 have no `[region]-s3` endpoint, and requests to their storage host return `501 NotImplemented`.
...
- **CORS Configuration**: CORS is not configurable — the S3 endpoint always responds with `Access-Control-Allow-Origin: *`. To control CORS, front the storage zone with a pull zone and configure it at the CDN level for file delivery.
...
- **Custom Metadata**: User-defined metadata (`x-amz-meta-*`) is not supported, so it is not carried over by `CopyObject`
...
| `NoSuchBucket` | 404 | The specified storage zone does not exist |
| `SignatureDoesNotMatch` | 403 | The request signature does not match (usually an incorrect secret access key) |
| `AccessDenied` | 403 | Missing or malformed `Authorization` header, or credentials that do not match the target storage zone |
| `PreconditionFailed` | 412 | A conditional header (`If-Match`, `If-Unmodified-Since`, `x-amz-copy-source-if-*`) did not hold |
| `BadDigest` | 400 | The supplied `x-amz-checksum-sha256` or `Content-MD5` does not match the uploaded data |
| `InvalidRequest` | 400 | Invalid request parameters or headers |
| `NotImplemented` | 501 | The requested operation is not implemented (for example `DeleteObjects`), or S3 is not supported by the storage region — see [Regional availability](#regional-availability) |
| `InternalError` | 500 | Internal server error |
| `ServiceUnavailable` | 403 | S3 compatibility is not enabled for this storage zone |
| `SlowDown` | 503 | Rate limit exceeded. The response includes a `Retry-After` header (in seconds) — wait for that interval, then retry with exponential backoff. See [Limits](#limits) |
...
3. **Implement retry logic** for transient failures, honouring the `Retry-After` header on `SlowDown` responses
...
9. **Read-only password support** use this secret access key for read-only access