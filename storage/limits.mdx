...
| Concurrent connections per zone | 100 | Up to 100 concurrent connections may upload to a single zone at the same time. |
...
FTP connections apply the following timeouts:

| Timeout | Value | Description |
| --- | --- | --- |
| Connection | 10 seconds | Applied when no connection is established. |
| Write | 10 seconds | Applied while writing data. |
| Read | 30 seconds | Applied while reading data. |
| Command read | 20 seconds | Applied while waiting for command results. |

## S3 throughput and rate limits
...
When you reach a connection or request limit, further requests will be rejected with:
