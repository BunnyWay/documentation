...
That creates the storage zone and the pull zone. It uploads each deploy to a directory of its own and publishes it instantly, so rolling back to an earlier one moves no files. It also reads three files out of your build: a `404.html` that answers a missing path, a `_redirects` file, and a `_headers` file. See [`bunny sites`](/cli/commands/sites).
