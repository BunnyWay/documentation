...
- **HEIC** *(beta)* - Compressed photo format commonly used by Apple devices
- **HEIF** *(beta)* - Image container with efficient compression
- **AVIF** *(beta)* - Image format using AV1 compression
...
- **AVIF** *(beta)* - Next-generation format with better compression than WebP and excellent quality, but slower encoding and slightly less universal support 
...
While AVIF can achieve even better compression, WebP is generally preferred for on-the-fly optimisation due to its significantly faster encoding times.
