...
<CardGroup cols={2}>
  <Card title="Wait for node recovery" icon="clock">
    If your data is important and the node's disk has not been replaced, wait for the node to return to service. Once the node is available again, your pod will automatically resume with the existing volume and all data intact.
  </Card>
...
If a node experiences hardware failure that requires disk replacement, any persistent volumes stored on that disk will be lost. The volume will be recreated as a new empty volume once the node is restored.

<Warning>
  Persistent volumes do not currently support backups or replication. If the underlying disk is replaced due to hardware failure, all data on that volume will be permanently lost. For critical data, implement your own backup strategy within your application.
</Warning>
