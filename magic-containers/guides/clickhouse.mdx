...
Then connect using the assigned hostname. In the examples below, `app` is both the username (`CLICKHOUSE_USER`) and the database name (`CLICKHOUSE_DB`) from the container's environment variables, and `YOUR_PASSWORD` is the `CLICKHOUSE_PASSWORD` you set. Substitute these with whatever you configured — for example, `clickhouse` for both if you kept the defaults.

<Tabs>
  <Tab title="Query">
    ```bash
    curl -u 'app:YOUR_PASSWORD' \
  ...
  <Tab title="Create">
    ```bash
    curl -u 'app:YOUR_PASSWORD' \
      'https://mc-abc123xyz0.b-cdn.net/?database=app' \
      --data-binary "
  ...
  <Tab title="Insert">
    ```bash
    curl -u 'app:YOUR_PASSWORD' \
      'https://mc-abc123xyz0.b-cdn.net/?database=app&query=INSERT+INTO+events+FORMAT+JSONEachRow' \
      --data-binary '
