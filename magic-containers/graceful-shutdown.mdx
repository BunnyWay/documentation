...
2. **Grace period** - Your application has 1 second by default to handle the signal, close connections, flush data, and exit cleanly.
...
<Info>
The grace period is configurable via the `terminationGracePeriodSeconds` API parameter. For applications that need more time to shut down gracefully, you can increase this value to match your requirements.
</Info>
...
- **Keep shutdown fast** - Aim to shut down as quickly as possible. If your application needs more time, configure the `terminationGracePeriodSeconds` parameter. Faster shutdowns reduce deployment times during rolling updates.
