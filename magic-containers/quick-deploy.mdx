...
**Quick Deploy** is a streamlined alternative to the standard deployment flow. Instead of stepping through multiple configuration screens, everything is presented in a single form, from picking your image to setting environment variables, so you can go from zero to deployed in seconds.
...
<Steps>
  ...
  <Step title="Deploy">
    Once everything looks good, click **Deploy**. Quick Deploy hands off to the same provisioning pipeline as a standard deployment. Your app will be created (or updated), the container scheduled, and endpoints activated.
