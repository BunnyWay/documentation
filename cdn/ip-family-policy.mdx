...
The bias is relaxed if the client is known to be IPv4. If [EDNS Client Subnet](#how-we-determine-the-client-address-family) tells us the client's address is IPv4, there is no benefit in preferring an IPv6-capable location, so selection falls back to normal distance and load based routing. AAAA queries keep the bias regardless, since only IPv6-capable clients act on an AAAA answer.
