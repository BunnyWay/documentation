...
If your origin requests its own certificates over ACME, Bunny forwards requests to `/.well-known/acme-challenge/` through to the origin so it can complete validation and have a certificate issued directly to it. Use this when the origin needs its own valid certificate.

## Custom certificate

Use this option for certificates from commercial providers, or for wildcard certificates when your domain is not managed by Bunny.
...
<Note>
  Manual upload is only needed for wildcards when your domain is hosted
  elsewhere. If the domain is managed in Bunny DNS, Bunny issues and renews Let's
  Encrypt wildcard certificates automatically. See [Wildcard
  certificates](#wildcard-certificates).
</Note>
