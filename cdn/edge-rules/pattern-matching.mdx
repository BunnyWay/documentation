...
# Pattern Matching in Edge Rules
...
Pattern matching is currently supported only for **Edge Rule actions
that execute at the Cache Layer**.

In the Edge Rules interface, these actions are marked with a **Cache**
badge.

Support for **Origin Layer actions** will be added in a future update.
...
Lua patterns provide a balance between expressive matching and
predictable performance.

Unlike full regular expressions, Lua patterns are implemented directly
in Lua's standard library and do not require a separate regex engine.
This reduces processing overhead and keeps rule evaluation fast.

Since Edge Rules are evaluated on every request at the edge, lightweight
matching is important to maintain performance across high request
volumes.
...
For more details on Lua patterns, see:
