...
Bunny Shield combines behavioral fingerprinting, header anomaly detection, and IP reputation to separate real users from automation without impacting legitimate visitors. Detection rules and sensitivity profiles propagate globally within seconds, so you can react to attacks as they unfold.
...
When bot detection (or another Shield feature) issues a challenge, the visitor is served a built-in JavaScript Proof-of-Work page that runs automatically in the browser. The challenge itself is not user-customizable, but if you have **whitelabelling enabled on your Pull Zone**, the Bunny branding is removed so the challenge page matches your domain without exposing the provider.

![Whitelabelled Shield challenge page](/images/shield/bot-detection/whitelabel-challenge.png)
