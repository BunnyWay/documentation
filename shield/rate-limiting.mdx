...
<Info>
  Rate limiting is not designed to guarantee an exact number of requests to your
  origin. Because Bunny Shield aggregates counters across our global network,
  there can be a short propagation delay, typically up to a few seconds before
  updated values are visible at every edge. During this time, some excess
