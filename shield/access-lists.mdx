...
Access Lists decide which clients can reach your origin based on identifiers like IP, CIDR range, ASN, or country. Curated threat feeds and your own custom rules run together at the edge, so policy changes apply globally without touching your backend.
...
- **Custom Lists**: Build your own access policies, from trusted networks through to high-risk regions.
...
Access Lists combine curated intelligence with your own rules:
...
- **Allow**: Pass trusted traffic through without restriction.
...
When multiple rules apply to the same request, Bunny Shield follows a defined order of precedence:
...
This order keeps trusted traffic from being blocked and applies security actions consistently across your rules.
...
- **Enterprise**: Includes all Advanced and Business feeds, plus support for custom feed integrations
