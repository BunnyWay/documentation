...
Every custom WAF rule is built from a small set of building blocks: a variable that tells the engine where to look in a request, an operator that defines the match condition, optional transformations that normalize the data before comparison, and a response action that determines what happens on a match. The sections below cover each piece in detail.
