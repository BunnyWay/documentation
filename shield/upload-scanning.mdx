...
Upload scanning runs inline with your traffic, in private, and across the file types your application accepts. All scanning happens inside the Bunny network, so data stays encrypted throughout.

## Antivirus scanning

All antivirus scanning is done entirely in-house, so no sensitive files leave the Bunny network.
