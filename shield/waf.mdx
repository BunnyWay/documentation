...
Predefined rules are tuned for general web traffic and can produce false positives on API-shaped requests where structured payloads, JWTs, or unusual content types resemble injection patterns. Rather than disabling individual managed rules, enabling [API Guardian](/shield/api-guardian) is often the cleaner option: for paths declared in your OpenAPI specification, API Guardian's schema-aware enforcement takes precedence over the predefined WAF ruleset, replacing it with validation aligned to your API contract. Paths outside the spec continue to be evaluated by the standard Shield protections, so the two features compose without overlap.
