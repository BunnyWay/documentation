...
<Tabs>
  ...
  <Tab title="CLI">
    ...
    Add records with `bunny dns records add`. Positional values follow the record type, and `'@'` targets the zone apex:
    ...
    # Link a record to a Pull Zone or DNS script
    bunny dns records add example.com cdn PullZone --pull-zone 12345
    bunny dns records add example.com fn Script --script 67890
    ```

    Omit the record type (or all arguments) to run an interactive wizard that prompts for the zone, type, and per-type values. Pass `--ttl` and `--comment` to set those fields.

    You can also apply a curated preset record set for a common provider or task (Google Workspace, Microsoft 365, DMARC, and more) in one step:

    ```bash
    bunny dns records preset list
...
<Tabs>
  ...
  <Tab title="CLI">
    Pass the record ID (from `bunny dns records list`), or omit it to pick the record interactively. Only the flags you set are changed:
...
<Tabs>
  ...
  <Tab title="CLI">
    Pass the record ID, or omit it to pick the record interactively. The CLI confirms before deleting unless you pass `--force`:
