...
`HTMLRewriter` lets you modify HTML responses as they stream through your edge
script. It parses the HTML on the fly and calls your handler functions when it
meets matching elements, comments, or text, so the whole document never has to
