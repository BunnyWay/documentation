...
BunnySDK.net.http.serve(async (request: Request): Response | Promise<Response> => {
  ...
  try {
    ...
    if (!res) {
      res = new Response(`<!DOCTYPE html><html><body><p>Hello <span id="name"></span></p><ul><li>created at: ${now.toISOString()}</li><li>expires at: ${expires.toISOString()}</li></ul></body></html>`, {
  ...
  try {
    ...
    if (context.request.method == "POST") {
      const res = Response(await `Content from POST: ${now.toISOString()}`, {
        headers: { "Cache-Control": "max-age=3600", "Content-Type": "text/plain; charset=utf-8" }
      });
