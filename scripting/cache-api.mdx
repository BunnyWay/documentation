...
<Info>
    **Note:** The caching API respects the following HTTP caching headers:
    - Cache-Control
    - If-None-Match
</Info>
...
`options` optional
- ignoreSearch
    - A boolean value that specifies whether the matching process should ignore the query string in the URL. If set to true, the ?value=bar part of `https://example.com/?value=bar` would be ignored when performing a match. It defaults to false.
- ignoreMethod
