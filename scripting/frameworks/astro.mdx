...
<Warning>
  Turn Optimizer on for the pull zone first. It is a paid feature, so the
  adapter never enables it for you, and the default stays `"noop"`. Without
  Optimizer the parameters are ignored and the original image is served.
</Warning>
...
| Prerendered pages, including `404.astro` | Yes, served from Storage |
