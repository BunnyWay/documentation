...
<Warning>
  The adapter is a lab project. It is on npm as
  [`@bunny.net/astro-adapter`](https://www.npmjs.com/package/@bunny.net/astro-adapter),
  at version 0.1.0. Pin the version you install, because its options and its
  build output can change in a minor release. Do not run it in production yet.
</Warning>
...
<Steps>
  ...
  <Step title="Run bunny lab deploy astro">
    ...
    The first prompt installs the adapter from npm. To add it yourself, run
    `npx astro add @bunny.net/astro-adapter` before you deploy.
