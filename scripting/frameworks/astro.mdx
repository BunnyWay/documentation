...
| What Astro built                     | What `bunny deploy` sends                                    |
| ------------------------------------ | ------------------------------------------------------------ |
...
A site whose routes are all prerendered needs no script, and no adapter. `bunny deploy` uploads the build, every deploy gets its own immutable URL, and nothing is invoked per request. The build says so:
