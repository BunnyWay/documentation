...
<Steps>
  ...
  <Step title="Run bunny deploy">
    ...
    ? Add @bunny.net/astro-adapter and render on the edge? › (Y/n)
    ✓ Set output: "server" and the adapter in astro.config.mjs.

    ℹ Running build: npm run build
    …
    [@bunny.net/astro-adapter] Bundled to dist/index.js (653 kB, limit 10 MB).
