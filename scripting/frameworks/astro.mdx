...
The script also passes `If-None-Match` and `If-Modified-Since` through, so a browser and the pull zone both revalidate with a `304` instead of downloading the object again.
...
<AccordionGroup>
  ...
  <Accordion title="A video is only seekable after it fully downloads">
    The file is not cached yet, so the pull zone fetches it as one large object.
    Turn on **Optimize for large object delivery** in the pull zone's caching
    settings, and it fetches the object in chunks instead.
  </Accordion>
