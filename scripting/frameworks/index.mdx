...
Each deploy is immutable, so a rollback is one step:

```bash
bunny deploy --prod     # publish to production
bunny deploy            # publish to the preview URL
bunny rollback          # back to the deploy that was live before
```

An adapter tells the CLI what it built, in a small manifest the build writes. The CLI reads the manifest, so every adapter deploys with the same commands, and a new adapter needs no new CLI.
