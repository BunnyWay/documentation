...
**One file.** A deployment is a single JavaScript file. Bundle the framework's server output with a bundler such as esbuild. A small Astro SSR build lands near 600 KB, well inside the 10 MB [script size limit](/scripting/limits).

**You supply the adapter.** There is no official Bunny adapter for any framework yet. Most frameworks let you write your own in a few dozen lines, because all it must do is hand you the app object. The [Astro guide](/scripting/frameworks/astro) shows a complete one.
...
<CardGroup cols={2}>
  <Card title="Astro" icon="rocket" href="/scripting/frameworks/astro">
    Run Astro's server at the edge with a custom adapter, or serve a static
    build with a small script.
  </Card>
</CardGroup>
