...
Unlike the browser Cache API, Bunny Edge Scripting does not support the options bag (`ignoreSearch`, `ignoreMethod`, `ignoreVary`). To get equivalent behaviour, normalize the key at `put()` time, for example by stripping query strings from the URL before calling `put()`.
...
- [Examples](./examples): end-to-end recipes for cache-aside, middleware writes, and refresh and purge.
