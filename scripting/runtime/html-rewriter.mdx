...
<Warning>
Our implementation is in preview as we are providing a complete in-house
implementation of HTMLRewriter.

`ExperimentalHTMLRewriter` and will be changed to `HTMLRewriter` before going public.
</Warning>


## Overview

`ExperimentalHTMLRewriter` lets you modify HTML responses as they stream through
...
BunnySDK.net.http.servePullZone()
  .onOriginResponse(async ({ response }) => {
    return new ExperimentalHTMLRewriter()
      .on("h1", {
        element(el) {
...
```js
new ExperimentalHTMLRewriter(options?)
```
